...
- **Bound qualification run: `ci/qualify.sh [--allow-dirty]`.** One command
  ...
  unless `--allow-dirty`, and a dirty run writes to
  `BuildEvidence/unqualified-<utc>/` — which `.gitignore` excludes — so a
  `qualified: false` manifest can never be swept into a commit by `git add -A`
  and read later as evidence.
...
- **Packages are deliberately minimal — 11 direct dependencies** (graph: 37
  entries). The template's feature packs (`characters-animation`,
  `gameplay-storytelling`, `worldbuilding`), the unused runtime packages
  (Behavior, Physics, Pipeline, Services, Timeline), the `feature.development`
  aggregate and `asset-manager-for-unity` were removed on
  `chore/package-reduction`, taking the player from 112M / 221 managed assemblies
  to 96M / 150. `feature.development` was replaced by explicit
  `com.unity.ide.rider` + `com.unity.ide.visualstudio` — the latter generates the
  `.csproj` files this repo's tooling reads, so it is load-bearing.
- **`ai.assistant` is kept but is NOT player-free**: it ships
  `Unity.AI.MCP.Runtime.dll` and `Unity.AI.Tracing.dll` (~38 KB) because it
  provides `Unity.AI.MCP.Editor`, the unity-mcp bridge. A minimal release player
  needs a manifest without it.
- **Two things are inert, and knowing that saves time:** `com.unity.modules.*`
  declarations do NOT control engine-module inclusion — removing
  `modules.assetbundle` or `modules.hierarchy` changes nothing and the module
  DLLs still ship (both were tested with a full qualification and reverted). And
  the four `Unity.Collections.Tests.CoreCLR.*` assemblies that ship are an
  upstream defect: `com.unity.collections` (arriving via URP, so not removable)
  omits `UNITY_INCLUDE_TESTS` from those asmdefs. Do not re-investigate either
  without new evidence. Details in `BuildEvidence/package-audit.txt`.
- Do not re-add a package, or claim one is editor-only, without checking
  `BuildEvidence/package-audit.txt` and re-running `ci/qualify.sh`.
- **Objectives are the progression record; the broadcast booleans are a cache.**
  `SaveValidator` derives the flags *downward* from `completedObjectives` and
  clears a flag they do not support. Do not "repair" this upward — a set flag
  inserting the objectives it implies lets a corrupt save manufacture
  progression. `SaveService.Load` likewise refuses while a scene transition is in
  flight, because committing state and then requesting a scene change is not
  atomic.
