...
- **Packages are deliberately minimal — 11 direct dependencies.** The template's
  ...
  player from 112M / 221 managed assemblies to 96M / 150. Two packages are kept
  on purpose: `asset-manager-for-unity` (the committed `uam/` tracking folder is
  generated by it; does not ship) and `ai.assistant` (it provides
  `Unity.AI.MCP.Editor`, the unity-mcp bridge). **`ai.assistant` is not
  player-free** — it ships `Unity.AI.MCP.Runtime.dll` and `Unity.AI.Tracing.dll`.
