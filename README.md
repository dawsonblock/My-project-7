...
The project is a small set of assemblies with one composition root. Runtime
components reach services through a single service locator
(`GameRoot.Instance.Services`) rather than through many scattered singletons —
that is still locator coupling, but it is one seam to reason about instead of
dozens, and it is what the command and event boundaries hang off.
...
- **The objectives are the progression record.** The two broadcast booleans are a
  cache of it, and `SaveValidator` derives *downward*: a flag the objectives do not
  support is cleared, rather than having objectives manufactured to match the flag.
  A corrupt save can therefore lose progress; it cannot invent it. Loading is also
  refused while a scene transition is in flight, because committing state and then
  asking for a scene change is not atomic and would leave `GameState` describing a
...
```bash
./ci/run-tests.sh all          # EditMode 86 + PlayMode 35, writes NUnit XML
...
Only runs that qualified something are tracked. A `--allow-dirty` run writes to
`BuildEvidence/unqualified-<utc>/`, which is git-ignored, so a `qualified: false`
manifest cannot be swept into a commit by `git add -A` and read later as evidence.
Every `qualification-<utc>/` directory in the repository is `qualified: true`.

## Package discipline

11 direct dependencies and 37 packages in the full graph, kept deliberately
minimal. The template's feature packs (`characters-animation`,
`gameplay-storytelling`, `worldbuilding`), the unused runtime packages (Behavior,
Physics, Pipeline, Services, Timeline), the `feature.development` aggregate and
`asset-manager-for-unity` were all removed once the gates were green — each cut
verified with a full `ci/qualify.sh` run, not by inspection:

| | Before | After |
|---|---|---|
| Player size | 112M | 96M |
| Managed assemblies | 221 | 150 |
| Direct packages | 20 | 11 |
| Full package graph | 76 | 37 |

Sizes are `du -sh` on the same machine. Unity player builds are not
bit-reproducible (embedded timestamps), so the exact byte count and hash for a
given build live in that run's manifest rather than in this table.

**Why the aggregate went.** `feature.development` was replaced by the two packages
it was actually standing in for — `com.unity.ide.rider` and
`com.unity.ide.visualstudio`, the latter being what generates the `.csproj` files
this repo's tooling reads. Declaring an aggregate because *some* of its contents
are useful hides the transitive surface behind a name that claims nothing.

**Two things are known and inert.** Recorded here so nobody spends an afternoon
re-investigating them:

- `com.unity.modules.assetbundle` and `com.unity.modules.hierarchy` have no
  measurable effect. Removed one at a time and fully qualified, they changed the
  player by -2.6 KB and -32 B with the same 150 assemblies, and
  `UnityEngine.AssetBundleModule.dll` / `UnityEngine.HierarchyModule.dll` shipped
  either way. A `com.unity.modules.*` declaration does **not** control engine-module
  inclusion in Unity 6000.6.0f1, so both entries are inert.
- Four `Unity.Collections.Tests.CoreCLR.*` assemblies ship that have no business
  being in a player. That is an **upstream defect**: `com.unity.collections` ships
  those helper assemblies with `defineConstraints: []` and `autoReferenced: true`,
  while its parent `Unity.Collections.Tests.asmdef` correctly requires
  `UNITY_INCLUDE_TESTS`. It arrives via `com.unity.render-pipelines.universal`, so
  it cannot be removed through the graph, and no project setting controls it.

**One deliberate cost.** `ai.assistant` provides `Unity.AI.MCP.Editor`, the bridge
behind the Unity MCP development tooling, and it is **not player-free**:
`Unity.AI.MCP.Runtime.dll` (9.7 KB) and `Unity.AI.Tracing.dll` (28 KB) are copied
into the player's `Managed/` directory. That is ~38 KB traded for a development
workflow rather than a free dependency — a minimal release player needs a manifest
without it.

`BuildEvidence/package-audit.txt` records the full reasoning; do not add a
dependency without checking it and re-running `ci/qualify.sh`.
