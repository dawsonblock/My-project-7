...
Two further packages are kept on purpose, and the accounting matters more than a
tidy story:

- **`asset-manager-for-unity`** — the committed `uam/` tracking folder is
  generated by it and the project tracks assets through it. It does not ship.
- **`ai.assistant`** — it provides `Unity.AI.MCP.Editor`, the bridge behind the
  Unity MCP development tooling. **It is not player-free:**
  `Unity.AI.MCP.Runtime.dll` (9.7 KB) and `Unity.AI.Tracing.dll` (28 KB) are both
  copied into the player's `Managed/` directory. Keeping it trades ~38 KB of
  shipped assemblies for a development workflow. For a minimal release player,
  build from a manifest without it rather than assuming it costs nothing.

`BuildEvidence/package-audit.txt` records the reasoning; do not add a dependency
without checking it and re-running `ci/qualify.sh`.
